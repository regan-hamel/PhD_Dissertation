...
dat <- data.frame(x = sce$sum, y = sce$detected, colour = sce$subsets_Mito_percent)
...
#Mito vs genes
dat <- data.frame(x = sce$detected, y = sce$subsets_Mito_percent, colour = sce$subsets_Mito_percent)
qcplot(dat$x, dat$y, data = dat, colour = colour, xlab = "UMI Counts", ylab = "% Mito Counts", col.label = "% Mito UMIs")

#Mito vs UMIs
dat <- data.frame(x = sce$sum, y = sce$subsets_Mito_percent, colour = sce$subsets_Mito_percent)
qcplot(dat$x, dat$y, data = dat, colour = sce$subsets_Mito_percent, xlab = "Gene Counts", ylab = "% Mito Counts", col.label = "% Mito UMIs") 

#Rib vs UMIs
#dat <- data.frame(x = sce$sum, y = sce$detected, colour = sce$pct_counts_Ribo)
#qcplot(dat$x, dat$y, data = dat, colour = colour, xlab = "Gene Counts", ylab = "% Ribo Counts", col.label = "% Ribo UMIs")

#Rib vs Mito
#dat <- data.frame(x = sce$pct_counts_Ribo, y = sce$subsets_Mito_percent, colour = sce$sum)
...
# sce$pct_features_Mito <- (sce$detected_Mito/sce$detected)*100
# #All 3 metrics - mito Genes
# dat <- data.frame(x = sce$sum, y = sce$detected, colour = sce$pct_features_Mito)
...
# 
# #ZOOM All 3 metrics - 1/2 cutoff
# dat <- data.frame(x = sce$total, y = sce$detected, colour = sce$pct_features_Mito)
